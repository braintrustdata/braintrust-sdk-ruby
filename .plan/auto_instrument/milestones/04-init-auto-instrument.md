...
module Braintrust
  class << self
    # @param auto_instrument [Boolean, Array<Symbol>, Hash] Auto-instrumentation config
    #   - true (default): instrument all available integrations
    #   - false: disable auto-instrumentation
    #   - Array: instrument only these integrations
...
Environment variables are combined with programmatic options:
- `only` is intersected (must be in both)
- `except` is unioned (excluded if in either)

## Tests to Create

### `test/braintrust/init_auto_instrument_test.rb`

- Test `init()` auto-instruments by default
- Test `init(auto_instrument: false)` skips instrumentation
