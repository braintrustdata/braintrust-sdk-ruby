  ...
  module Server
    class RackTest < Minitest::Test
      ...
      def test_app_returns_cors_wrapping_auth_wrapping_router
        app = Rack.app(evaluators: {}, auth: :none)

        assert_instance_of Middleware::Cors, app

        auth_middleware = app.instance_variable_get(:@app)
        assert_instance_of Middleware::Auth, auth_middleware
      ...
      def test_app_uses_no_auth_strategy_when_none
        ...
        assert_instance_of Auth::NoAuth, strategy
      end

      def test_app_uses_clerk_token_strategy_by_default
        app = Rack.app(evaluators: {})

        auth_middleware = app.instance_variable_get(:@app)
        strategy = auth_middleware.instance_variable_get(:@strategy)

        assert_instance_of Auth::ClerkToken, strategy
      end

      def test_app_accepts_custom_auth_strategy
        custom_auth = Auth::NoAuth.new

        app = Rack.app(evaluators: {}, auth: custom_auth)
