...
class Braintrust::Eval::EvaluatorTest < Minitest::Test
  ...
  def test_run_delegates_to_eval_run
    ...
    result = evaluator.run(cases, quiet: true)
  ...
  def test_run_passes_on_progress
    ...
    evaluator.run(cases, on_progress: ->(data) { progress_events << data }, quiet: true)
  ...
  def test_subclass_run
    ...
    evaluator = klass.new
    result = evaluator.run([{input: "hello", expected: "HELLO"}], quiet: true)
