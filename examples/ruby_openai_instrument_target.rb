...
# Example: ruby-openai (alexrudall) chat completion with Braintrust tracing (instance-level)
#
# This example demonstrates how to automatically trace OpenAI API calls using
# the ruby-openai gem (by alexrudall) with Braintrust instance-level instrumentation.
# Only the specific client passed to instrument! is traced.
#
# Note: ruby-openai is an optional development dependency. To run this example:
#   1. Install ruby-openai: gem install ruby-openai
#   2. Run from the SDK root: ruby examples/ruby_openai_instrument_target.rb
#
# Usage:
#   OPENAI_API_KEY=your-openai-key ruby examples/ruby_openai_instrument_target.rb
...
# Instrument only this specific client
...
response = tracer.in_span("examples/ruby_openai_instrument_target.rb") do |span|
  root_span = span
